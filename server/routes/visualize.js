...
// POST /admin-cache/:authorId/:workId — admin-only: generate canonical word
// pictures for a work and save them to shakespeare.json so all users get them.
// Auth via X-Admin-Key header matching ADMIN_KEY. Used once per work at rollout
// time; safe to re-run (overwrites the canonical cache for that work).
router.post('/admin-cache/:authorId/:workId', async (req, res) => {
  ...
  try {
    ...
    console.log(`[admin-cache ${workId}] Step 1: generating word pictures...`);
    const initialPictures = await callGroqAPI(buildWordPicturesPrompt(work, dramaticContext), apiKey);

    // Step 2: reflect + improve (optional; fall back to initial on failure)
    let finalPictures = initialPictures;
    try {
      console.log(`[admin-cache ${workId}] Step 2: reflecting...`);
      finalPictures = await callGroqAPI(buildReflectionPrompt(work, initialPictures, dramaticContext), apiKey);
    } catch (err) {
    ...
    res.json({
      success: true,
      workId,
      chunks: Object.keys(finalPictures).length,
      source: 'admin-cache'
    });
  } catch (err) {
    console.error(`[admin-cache ${workId}] error:`, err);
...
router.get('/word-pictures/:authorId/:workId', validateKey, async (req, res) => {
  try {
    ...
    // Load existing word pictures — merge canonical (from shakespeare.json,
    // pre-generated by admin) with per-user overrides. Canonical fills the
    // "generated" pool with a shared set of image options; per-user overrides
    // win on a chunk-by-chunk basis when the user has regenerated. selected/
    // rooms remain per-user only (those are personal choices).
    ...
    res.json({
      ...
      wordInitialGrid,
      wordPictures: mergedWordPictures
    });
  } catch (err) {
